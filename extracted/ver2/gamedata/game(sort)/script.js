    ...
    fs.innerHTML = `
      ...
      <label class="mode-opt">
        ...
        <span class="mode-body">
          ...
          <span class="mode-desc">正解するたびに時間がふえる（最長90秒）</span>
        </span>
      </label>`;
...
const MAX_BONUS_TIME = 30;   // 1プレイで増やせる時間の合計（秒）＝最長90秒
const PAIR_TIME_BONUS = 2;   // ペアをそろえたときの加算（秒）
function comboTimeBonus(combo) {
  return combo >= 10 ? 2 : combo >= 5 ? 1.5 : 1;
}
// 上限まで時間を足す。実際に足せた秒数を返す
function grantBonusTime(sec) {
  const room = Math.max(0, MAX_BONUS_TIME - bonusTimeGained);
  const add = Math.min(sec, room);
