...
let wordIdCounter = 0;
let selectedCategories = new Set(); // 出題するカテゴリ（加減法、乗法…）
...
/* ===============================
   ランキング
=============================== */
function getLocalKey()      { return "ranksMath符号" + (bonusEnabled ? "" : "_nobonus"); }
function getCollectionName(){ return "ranksMath符号" + (bonusEnabled ? "" : "_nobonus"); }
...
async function displayGlobalRanking() {
  ...
  const db = getDb();
  if (!db) { tbody.innerHTML = "<tr><td colspan='3'>Firebase未接続</td></tr>"; return; }
  const { getDocs, query, collection, orderBy, limit } = getFbModules();
  try {
    const qSnap = await getDocs(query(collection(db, getCollectionName()), orderBy("score", "desc"), limit(300)));
    tbody.innerHTML = "";
    const seenDevices = new Set();
    let count = 0;
    for (const docSnap of qSnap.docs) {
      const { player, score: sc, deviceId } = docSnap.data();
      if (deviceId && seenDevices.has(deviceId)) continue;
      if (deviceId) seenDevices.add(deviceId);
      count++;
      if (count > 30) break;
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${count}</td><td>${player}</td><td>${sc}</td>`;
      tbody.appendChild(tr);
    }
...
async function saveGlobalScore(username, sc) {
  ...
  try {
    await addDoc(collection(db, getCollectionName()), { player: username, score: sc, date: today, deviceId });
  } catch (e) {
...
function createSortingArea() {
  ...
  SORT_CATEGORIES.forEach(({ id, label }) => {
    ...
    col.innerHTML = `<span class="sorting-label">${label}</span>`;
...
function spawnWord(presetX) {
  ...
  if (presetX !== undefined) {
    ...
    let overlap = fallingWords.some(w => x < w.x + w.element.offsetWidth && x + measuredWidth > w.x);
...
function handleMouseUp(e) {
  ...
  if (top >= getDecisionLineY() && wordElem.dataset.locked === "false") {
    ...
    if (wordElem.dataset.type === dropCategory) {
      ...
      wrongAnswers.push({ expr: wordElem.textContent, correctType: wordElem.dataset.type });
    }
  }
  currentDrag = null;
}
    ...
    if (newY >= decisionLineY) {
      ...
      if (word.element.dataset.type === dropCategory) {
        ...
        if (word.element.dataset.penalized !== "true") {
          ...
          currentCombo = 0;
          updateComboDisplay();
          wrongAnswers.push({ expr: word.element.textContent, correctType: word.element.dataset.type });
        }
